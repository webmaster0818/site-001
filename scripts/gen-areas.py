...
def gen(city, w):
    ...
    city_level = city.get("level") == "city"
    full = cityname if city_level else f"{cityname}{ward}"
    ...
    if city_level:
        s = s.replace('<li><Link href="/areas/chiba/">千葉県</Link></li>', f'<li>{pref}</li>')
    else:
...
if __name__ == "__main__":
    import importlib
    mod = "area_data_groupA"
    for a in sys.argv[1:]:
        if a.startswith("group"):
            mod = "area_data_" + a
    CITIES = importlib.import_module(mod).CITIES
